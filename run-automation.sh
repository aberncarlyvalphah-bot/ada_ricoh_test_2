...
3. Test thoroughly:
   - For page/component changes: Use Puppeteer screenshot.js to verify UI in browser
   - For minor changes: Run npm run lint and npm run build
   - Verify all steps in the task description
...
- Mark the task as passes: true in task.json
- Document your work in progress.txt
- Commit all changes (code, progress.txt, task.json) together
PROMPT_EOF
    ...
    if claude -p \
        ...
        --allowed-tools "Bash Edit Read Write Glob Grep Skill WebSearch WebFetch mcp__playwright__* mcp__4_5v_mcp__analyze_image mcp__web-reader__webReader mcp__web-search-prime__webSearchPrime" \
